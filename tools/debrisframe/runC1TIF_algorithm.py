...
class runC1TIFAlgorithm(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if sourceRELCSV:
            relCSVDir = targetDir / "Inputs" / "REL"
            relCSVDir.mkdir(parents=True, exist_ok=True)
            shutil.copy(sourceRELCSV, relCSVDir / pathlib.Path(sourceRELCSV).name)
        ...
        try:
            ...
            raise QgsProcessingException(
                f"{self.tr('Something went wrong with c1TIF, please check log files')}\n{err}"
            )
